...
\begin{abstract}
  ...
  operations over intervals and interval sets.
\end{abstract}
...
Fig.~4 --- as a first-class operation, \texttt{Tempo.compose/2}: given
$A\;r_1\;B$ and $B\;r_2\;C$, it returns the set of relations that can
hold between $A$ and $C$. Each of the 169 cells was derived
independently, by a difference-bound consistency check over the six
endpoints under the half-open convention, and then checked
cell-for-cell against the published table. Composition is at present a
standalone query rather than the engine of a propagation algorithm ---
see \S\ref{sec:discussion} --- but it is the piece on which qualitative
reasoning over non-grounded intervals would be built.
...
\begin{enumerate}
  ...
  \item \textbf{Allen's algebra over interval sets.} The thirteen
    ...
    (existential, universal, set-of-relations). The library now offers
    two of the three --- the existential lifting through the
    predicates, and the set-of-relations lifting through
    \texttt{relation\_matrix/2}, which returns the relation holding
    between every pair of members --- but which should be the
    \emph{default} is not obvious, and a principled extension of the
    ontology to non-convex interval sets would settle it.
